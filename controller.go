...
import (
	...
	foov1alpha1 "github.com/LiangNing7/foo-controller/pkg/apis/foocontroller/v1alpha1"
...
func (c *Controller) Run(ctx context.Context, workers int) error {
	...
	// Start the informer factories to begin population the informer caches.
...
func (c *Controller) syncHandler(ctx context.Context, objectRef cache.ObjectName) error {
	...
	if err != nil {
		...
		if errors.IsNotFound(err) {
			utilruntime.HandleErrorWithContext(ctx, err, "Foo reference by items in work queue no longer exist", "objectReference", objectRef)
			return nil // return nil do't try.
		}

		return err // return err try again.
	}
...
func (c *Controller) updateFooStatus(ctx context.Context, foo *foov1alpha1.Foo, deployment *appsv1.Deployment) error {
	...
	_, err := c.fooclientset.FoocontrollerV1alpha1().Foos(foo.Namespace).UpdateStatus(ctx, fooCopy, metav1.UpdateOptions{FieldManager: FieldManager})
	return err
}
...
func newDeployment(foo *foov1alpha1.Foo) *appsv1.Deployment {
	...
	return &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			...
			OwnerReferences: []metav1.OwnerReference{
				*metav1.NewControllerRef(foo, foov1alpha1.SchemeGroupVersion.WithKind("Foo")),
			},
		},
